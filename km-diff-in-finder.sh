...
# BEGINMSG
# This is an optional function which uses growlnotify, if installed. You can find it as part of the github repo at https://github.com/tjluoma/km-diff-dropbox-duplicates
	ZMSG="${HOME}/.dotfiles/bin/repos/km-diff-dropbox-duplicates"
...
then
	# This variable is set from the Keyboard Maestro Macro 'Finder-BBDiff-Dropbox-Duplicate-Files.kmmacros' if we're being called from Keyboard Maestro then re-call the script with an arg
...
# if the `diff` command has an option to 'wait' (ksdiff and bbdiff both use --wait) and you want to use it, include it here
DIFF_WAIT='--wait'
...
do
	...
	# and then a literal )
	#
	# However, if you have a file named something like "My File (draft) (Joe's conflicted copy 2013-03-12).tld" this regex will remove the "(draft)" too. I never use ( ) in my filenames so I can live with that.
	#
	...
	ORIGINAL_FILE=$(echo "$i" | sed "s# (.*'s conflicted copy [0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9].*)##g")
		...
		if [[ -e "$ORIGINAL_FILE" ]]
		then # First, check whether the files are identical

			if (( cmp -s "$ORIGINAL_FILE" "$i" ))
			then # the files are identical, so trash the "conflicted copy"

				if (( $+commands[trash] ))
				then # if the `trash` command is installed, use it

					trash "$i" && msg "Files are identical, trashed duplicate."

				else # if the trash command isn't installed, just use `mv`
			...
			# if we did not find an "original" file… well, then this isn't really a duplicate or a conflict, so we should rename it to the original name without the extra bits added by Dropbox.
			msg sticky "No $ORIGINAL_FILE found for $i"

			# Change the ID so it doesn't overwrite the previous 'sticky' message
			GROWL_ID="$i.2"

			# rename the file and then tell the user we renamed the file and then show the file in the Finder
